...
package LPC43_SVD.MCPWM is
   ...
   subtype CON_RESERVED_Field is HAL.UInt3;
   subtype CON_RESERVED_Field_1 is HAL.UInt8;
   ...
   subtype CON_SET_RESERVED_Field is HAL.UInt3;
   subtype CON_SET_RESERVED_Field_1 is HAL.UInt8;

   --  PWM Control set address
   type CON_SET_Register is record
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      RUN0_SET    : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      CENTER0_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      POLA0_SET   : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      DTE0_SET    : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      DISUP0_SET  : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      RESERVED    : CON_SET_RESERVED_Field := 16#0#;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      RUN1_SET    : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      CENTER1_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      POLA1_SET   : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      DTE1_SET    : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      DISUP1_SET  : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      RESERVED_1  : CON_SET_RESERVED_Field := 16#0#;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      RUN2_SET    : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      CENTER2_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      POLA2_SET   : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      DTE2_SET    : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      DISUP2_SET  : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      RESERVED_2  : CON_SET_RESERVED_Field_1 := 16#0#;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      INVBDC_SET  : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      ACMODE_SET  : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CON
      --  register.
      DCMODE_SET  : Boolean := False;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   subtype CON_CLR_RESERVED_Field is HAL.UInt3;
   subtype CON_CLR_RESERVED_Field_1 is HAL.UInt8;

   --  PWM Control clear address
   type CON_CLR_Register is record
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      RUN0_CLR    : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      CENTER0_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      POLA0_CLR   : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      DTE0_CLR    : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      DISUP0_CLR  : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      RESERVED    : CON_CLR_RESERVED_Field := 16#0#;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      RUN1_CLR    : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      CENTER1_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      POLA1_CLR   : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      DTE1_CLR    : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      DISUP1_CLR  : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      RESERVED_1  : CON_CLR_RESERVED_Field := 16#0#;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      RUN2_CLR    : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      CENTER2_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      POLA2_CLR   : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      DTE2_CLR    : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      DISUP2_CLR  : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      RESERVED_2  : CON_CLR_RESERVED_Field_1 := 16#0#;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      INVBDC_CLR  : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      ACMOD_CLR   : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CON
      --  register.
      DCMODE_CLR  : Boolean := False;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   --  CAPCON_RT array
   type CAPCON_RT_Field_Array is array (0 .. 2) of Boolean
   ...
   subtype CAPCON_RESERVED_Field is HAL.UInt11;
   ...
   subtype CAPCON_SET_RESERVED_Field is HAL.UInt11;
   ...
   type CAPCON_SET_Register is record
      ...
      --  register.
      CAP0MCI0_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP0MCI0_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP0MCI1_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP0MCI1_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP0MCI2_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP0MCI2_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP1MCI0_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP1MCI0_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP1MCI1_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP1MCI1_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP1MCI2_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP1MCI2_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP2MCI0_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP2MCI0_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP2MCI1_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP2MCI1_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP2MCI2_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      CAP2MCI2_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      RT0_SET         : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      RT1_SET         : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bits in the CAPCON
      --  register.
      RT2_SET         : Boolean := False;
      --  Write-only. Reserved.
   ...
   subtype CAPCON_CLR_RESERVED_Field is HAL.UInt11;
   ...
   type CAPCON_CLR_Register is record
      ...
      --  register.
      CAP0MCI0_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP0MCI0_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP0MCI1_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP0MCI1_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP0MCI2_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP0MCI2_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP1MCI0_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP1MCI0_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP1MCI1_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP1MCI1_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP1MCI2_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP1MCI2_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP2MCI0_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP2MCI0_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP2MCI1_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP2MCI1_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP2MCI2_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      CAP2MCI2_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      RT0_CLR         : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      RT1_CLR         : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bits in the CAPCON
      --  register.
      RT2_CLR         : Boolean := False;
      --  Write-only. Reserved.
   ...
   --  Timer Counter register
   type TC_Registers is array (0 .. 2) of HAL.UInt32
     with Volatile;

   --  Limit register

   --  Limit register
   type LIM_Registers is array (0 .. 2) of HAL.UInt32
     with Volatile;

   --  Match register

   --  Match register
   type MAT_Registers is array (0 .. 2) of HAL.UInt32
     with Volatile;

   --  DT array element
   subtype DT_Element is HAL.UInt10;
   ...
   subtype DT_RESERVED_Field is HAL.UInt2;
   ...
   subtype CCP_RESERVED_Field is HAL.UInt26;
   ...
   --  Capture register
   type CAP_Registers is array (0 .. 2) of HAL.UInt32
     with Volatile;
   ...
   subtype INTEN_RESERVED_Field is HAL.UInt4;
   subtype INTEN_RESERVED_Field_1 is HAL.UInt16;
   ...
   type INTEN_Register is record
      ...
      ICAP2      : INTEN_ENUM;
      --  Read-only. Reserved.
      RESERVED_2 : INTEN_RESERVED_Field;
      --  Read-only. Fast abort interrupt.
      ABORT_k    : INTEN_ENUM;
      --  Read-only. Reserved.
      RESERVED_3 : INTEN_RESERVED_Field_1;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   subtype INTEN_SET_RESERVED_Field is HAL.UInt3;
   subtype INTEN_SET_RESERVED_Field_1 is HAL.UInt16;
   ...
   type INTEN_SET_Register is record
      ...
      ILIM0_SET    : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in INTEN, thus
      --  enabling the interrupt.
      IMAT0_SET    : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in INTEN, thus
      --  enabling the interrupt.
      ICAP0_SET    : Boolean := False;
      --  Write-only. Reserved.
      RESERVED     : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in INTEN, thus
      --  enabling the interrupt.
      ILIM1_SET    : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in INTEN, thus
      --  enabling the interrupt.
      IMAT1_SET    : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in INTEN, thus
      --  enabling the interrupt.
      ICAP1_SET    : Boolean := False;
      --  Write-only. Reserved.
      ...
      --  Write-only. Writing a one sets the corresponding bit in INTEN, thus
      --  enabling the interrupt.
      ILIM2_SET    : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in INTEN, thus
      --  enabling the interrupt.
      IMAT2_SET    : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in INTEN, thus
      --  enabling the interrupt.
      ICAP2_SET    : Boolean := False;
      --  Write-only. Reserved.
      RESERVED_2   : INTEN_SET_RESERVED_Field := 16#0#;
      --  Write-only. Writing a one sets the corresponding bit in INTEN, thus
      --  enabling the interrupt.
      ABORT_SET    : Boolean := False;
      --  Write-only. Reserved.
      RESERVED_3   : INTEN_SET_RESERVED_Field_1 := 16#0#;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   subtype INTEN_CLR_RESERVED_Field is HAL.UInt4;
   subtype INTEN_CLR_RESERVED_Field_1 is HAL.UInt16;
   ...
   type INTEN_CLR_Register is record
      ...
      ILIM0_CLR  : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      IMAT0_CLR  : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      ICAP0_CLR  : Boolean := False;
      --  Write-only. Reserved.
      RESERVED   : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      ILIM1_CLR  : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      IMAT1_CLR  : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      ICAP1_CLR  : Boolean := False;
      --  Write-only. Reserved.
      RESERVED_1 : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      ILIM2_CLR  : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      IMAT2_CLR  : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      ICAP2_CLR  : Boolean := False;
      --  Write-only. Reserved.
      RESERVED_2 : INTEN_CLR_RESERVED_Field := 16#0#;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      ABORT_CLR  : Boolean := False;
      --  Write-only. Reserved.
      RESERVED_3 : INTEN_CLR_RESERVED_Field_1 := 16#0#;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   subtype CNTCON_RESERVED_Field is HAL.UInt11;
   ...
   subtype CNTCON_SET_RESERVED_Field is HAL.UInt11;
   ...
   type CNTCON_SET_Register is record
      ...
      --  register.
      TC0MCI0_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC0MCI0_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC0MCI1_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC0MCI1_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC0MCI2_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC0MCI2_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC1MCI0_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC1MCI0_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC1MCI1_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC1MCI1_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC1MCI2_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC1MCI2_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC2MCI0_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC2MCI0_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC2MCI1_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC2MCI1_FE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC2MCI2_RE_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      TC2MCI2_FE_SET : Boolean := False;
      --  Write-only. Reserved.
      RESERVED       : CNTCON_SET_RESERVED_Field := 16#0#;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      CNTR0_SET      : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      CNTR1_SET      : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the CNTCON
      --  register.
      CNTR2_SET      : Boolean := False;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   subtype CNTCON_CLR_RESERVED_Field is HAL.UInt11;
   ...
   type CNTCON_CLR_Register is record
      ...
      --  register.
      TC0MCI0_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC0MCI0_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC0MCI1_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC0MCI1_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC0MCI2_RE     : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC0MCI2_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC1MCI0_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC1MCI0_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC1MCI1_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC1MCI1_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC1MCI2_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC1MCI2_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC2MCI0_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC2MCI0_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC2MCI1_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC2MCI1_FE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC2MCI2_RE_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      TC2MCI2_FE_CLR : Boolean := False;
      --  Write-only. Reserved.
      RESERVED       : CNTCON_CLR_RESERVED_Field := 16#0#;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      CNTR0_CLR      : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      CNTR1_CLR      : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in the CNTCON
      --  register.
      CNTR2_CLR      : Boolean := False;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   subtype INTF_RESERVED_Field is HAL.UInt4;
   subtype INTF_RESERVED_Field_1 is HAL.UInt16;
   ...
   type INTF_Register is record
      ...
      ICAP2_F    : INTF_ENUM;
      --  Read-only. Reserved.
      RESERVED_2 : INTF_RESERVED_Field;
      --  Read-only. Fast abort interrupt flag.
      ABORT_F    : INTF_ENUM;
      --  Read-only. Reserved.
      RESERVED_3 : INTF_RESERVED_Field_1;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   subtype INTF_SET_RESERVED_Field is HAL.UInt4;
   subtype INTF_SET_RESERVED_Field_1 is HAL.UInt16;
   ...
   type INTF_SET_Register is record
      ...
      ILIM0_F_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the INTF
      --  register, thus possibly simulating hardware interrupt.
      IMAT0_F_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the INTF
      --  register, thus possibly simulating hardware interrupt.
      ICAP0_F_SET : Boolean := False;
      --  Write-only. Reserved.
      RESERVED    : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the INTF
      --  register, thus possibly simulating hardware interrupt.
      ILIM1_F_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the INTF
      --  register, thus possibly simulating hardware interrupt.
      IMAT1_F_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the INTF
      --  register, thus possibly simulating hardware interrupt.
      ICAP1_F_SET : Boolean := False;
      --  Write-only. Reserved.
      RESERVED_1  : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the INTF
      --  register, thus possibly simulating hardware interrupt.
      ILIM2_F_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the INTF
      --  register, thus possibly simulating hardware interrupt.
      IMAT2_F_SET : Boolean := False;
      --  Write-only. Writing a one sets the corresponding bit in the INTF
      --  register, thus possibly simulating hardware interrupt.
      ICAP2_F_SET : Boolean := False;
      --  Write-only. Reserved.
      RESERVED_2  : INTF_SET_RESERVED_Field := 16#0#;
      --  Write-only. Writing a one sets the corresponding bit in the INTF
      --  register, thus possibly simulating hardware interrupt.
      ABORT_F_SET : Boolean := False;
      --  Write-only. Reserved.
      RESERVED_3  : INTF_SET_RESERVED_Field_1 := 16#0#;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   subtype INTF_CLR_RESERVED_Field is HAL.UInt4;
   subtype INTF_CLR_RESERVED_Field_1 is HAL.UInt16;
   ...
   type INTF_CLR_Register is record
      ...
      ILIM0_F_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      IMAT0_F_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      ICAP0_F_CLR : Boolean := False;
      --  Write-only. Reserved.
      RESERVED    : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      ILIM1_F_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      IMAT1_F_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      ICAP1_F_CLR : Boolean := False;
      --  Write-only. Reserved.
      RESERVED_1  : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      ILIM2_F_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      IMAT2_F_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      ICAP2_F_CLR : Boolean := False;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      RESERVED_2  : INTF_CLR_RESERVED_Field := 16#0#;
      --  Write-only. Writing a one clears the corresponding bit in INTEN, thus
      --  disabling the interrupt.
      ABORT_F_CLR : Boolean := False;
      --  Write-only. Reserved.
      RESERVED_3  : INTF_CLR_RESERVED_Field_1 := 16#0#;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   --  CAP_CLR array
   type CAP_CLR_Field_Array is array (0 .. 2) of Boolean
     with Component_Size => 1, Size => 3;
   ...
   subtype CAP_CLR_RESERVED_Field is HAL.UInt29;
