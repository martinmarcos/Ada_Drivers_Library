...
package NRF_SVD.SCU is
   ...
   subtype CSCR_EOSCCON_Field is HAL.UInt2;
   ...
   type CSCR_Register is record
      ...
      EOSCCON       : CSCR_EOSCCON_Field := 16#0#;
      --  unspecified
   ...
   subtype NMIR_ACCESSCODE_Field is HAL.UInt16;

   --  NMI Control Register
   type NMIR_Register is record
      --  LVD Condition Enable
      LVDEN          : Boolean := False;
      --  MCLK Fail Condition Enable
      MCLKFAILEN     : Boolean := False;
      --  Watch Dog Interrupt Condition Enable
      WDTEN          : Boolean := False;
      --  Over Voltage Protection Condition Enable
      OVPEN          : Boolean := False;
      --  Protection Condition Enable
      PROTEN         : Boolean := False;
      --  unspecified
      Reserved_5_7   : HAL.UInt3 := 16#0#;
      --  LVD Condition Status
      LVDSTS         : Boolean := False;
      --  MCLK Fail Condition Status
      MCLKFAILSTS    : Boolean := False;
      --  Watch Dog Interrupt Condition Status
      WDTSTS         : Boolean := False;
      --  Over Voltage Protection Condition Status
      OVPSTS         : Boolean := False;
      --  Protection Condition Status
      PROTSTS        : Boolean := False;
      --  unspecified
      Reserved_13_15 : HAL.UInt3 := 16#0#;
      --  Write-only. Access Code
      ACCESSCODE     : NMIR_ACCESSCODE_Field := 16#0#;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   type VDCCON_Register is record
      ...
      VDCWDLY        : VDCCON_VDCWDLY_Field := 16#7F#;
      --  Write-only. VDCDE
      VDCDE          : Boolean := False;
      --  unspecified
