...
package NRF_SVD.TIMER is
   ...
   type CR2_Register is record
      ...
      --  Read-only. Timer PWM Output Flag
      PWMO          : Boolean := False;
      --  unspecified
   ...
   --  Timer n General Register A (Duty)
   ...
   --  Timer n General Register B (Period)
   ...
   subtype CNT_PRECLR_Field is HAL.UInt16;

   --  Timer n Count Register
   type CNT_Register is record
      --  CNT
      CNT    : CNT_CNT_Field := 16#0#;
      --  Prescaler Initialized for counting
      PRECLR : CNT_PRECLR_Field := 16#0#;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;
