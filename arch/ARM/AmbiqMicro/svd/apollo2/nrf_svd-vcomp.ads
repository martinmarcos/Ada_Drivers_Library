...
package NRF_SVD.VCOMP is
   ...
   type CFG_PSEL_Field is
     (
      ...
      Vddadj,
      --  Use the temperature sensor output for the positive input. Note: If
      --  this channel is selected for PSEL, the bandap circuit required for
      --  temperature comparisons will automatically turn on. The bandgap
      --  circuit requires 11us to stabalize. value.
      Vtemp,
      --  Use external voltage 0 for positive input. value.
      Vext1,
   ...
   type CFG_NSEL_Field is
     (
      ...
      Vrefext3,
      --  Use DAC output selected by LVLSEL for reference input. value.
      Dac)
     with Size => 2;
