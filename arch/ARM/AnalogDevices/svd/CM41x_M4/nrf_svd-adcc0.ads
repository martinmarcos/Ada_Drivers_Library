...
package NRF_SVD.ADCC0 is
   ...
   type ERRMSK_SET_Register is record
      --  Trigger Overrun 0 Mask Set
      TRGOV         : ERRMSK_SET_TRGOV_Field :=
                       (As_Array => False, Val => 16#0#);
      --  Bandwidth Monitor Error 0 Mask Set
      BWERR         : ERRMSK_SET_BWERR_Field :=
                       (As_Array => False, Val => 16#0#);
      --  Memory Response Error 0 Mask Set
      MERR          : ERRMSK_SET_MERR_Field :=
                       (As_Array => False, Val => 16#0#);
      --  Event Collision Error Mask Set
      ECOL          : Boolean := False;
      --  Event Miss Error Mask Set
      EMIS          : Boolean := False;
      --  Event data Overflow Error Mask Set
      DATOVF        : Boolean := False;
      --  unspecified
      Reserved_9_31 : HAL.UInt23 := 16#0#;
   end record
   ...
   type ERRMSK_CLR_Register is record
      --  Trigger Overrun 0 Mask Clear
      TRGOV         : ERRMSK_CLR_TRGOV_Field :=
                       (As_Array => False, Val => 16#0#);
      --  Bandwidth Monitor Error 0 Mask Clear
      BWERR         : ERRMSK_CLR_BWERR_Field :=
                       (As_Array => False, Val => 16#0#);
      --  Memory Response Error 0 Mask Clear
      MERR          : ERRMSK_CLR_MERR_Field :=
                       (As_Array => False, Val => 16#0#);
      --  Event Collision Error Mask Clear
      ECOL          : Boolean := False;
      --  Event Miss Error Mask Clear
      EMIS          : Boolean := False;
      --  Event data OVerflow Error Mask Clear
      DATOVF        : Boolean := False;
      --  unspecified
      Reserved_9_31 : HAL.UInt23 := 16#0#;
   end record
