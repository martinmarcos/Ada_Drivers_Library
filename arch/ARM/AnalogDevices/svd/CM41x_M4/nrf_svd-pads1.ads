...
package NRF_SVD.PADS1 is
   ...
   --  Fast Over Current Protection Clock Divisor
   ...
   type DBC_CTL_Register is record
      ...
      --  disabled)
      EN             : Boolean := False;
      --  Debounce Filter Decimating (or Continuous) Sample Mode
      DEC            : Boolean := False;
      --  Prescale Clock Select(or System Clock=Default)
      PRECLKSEL      : Boolean := False;
      --  Accumulator Mode(Counter Mode is Default)
