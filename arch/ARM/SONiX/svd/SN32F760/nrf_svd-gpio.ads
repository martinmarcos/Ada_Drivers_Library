...
package NRF_SVD.GPIO is
   ...
   type MODE_CURRENT0_Field is
     (
      --  Typical 10mA
      MODE_CURRENT0_Field_10MA,
      --  Typical 20mA
      MODE_CURRENT0_Field_20MA)
     with Size => 1;
   for MODE_CURRENT0_Field use
     (MODE_CURRENT0_Field_10MA => 0,
      MODE_CURRENT0_Field_20MA => 1);

   --  MODE_CURRENT array
   type MODE_CURRENT_Field_Array is array (0 .. 15) of MODE_CURRENT0_Field
     with Component_Size => 1, Size => 16;

   --  Type definition for MODE_CURRENT
   type MODE_CURRENT_Field
   ...
   is record
      case As_Array is
         ...
         when True =>
            --  CURRENT as an array
            Arr : MODE_CURRENT_Field_Array;
      end case;
   end record
     with Unchecked_Union, Size => 16;

   for MODE_CURRENT_Field use record
      Val at 0 range 0 .. 15;
      Arr at 0 range 0 .. 15;
   end record;
   ...
   type MODE_Register is record
      ...
      --  Driving/Sinking current of Pn.0
      CURRENT : MODE_CURRENT_Field := (As_Array => False, Val => 16#0#);
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   type ODCTRL_OC0_Field is
     (
      ...
      Enable)
     with Size => 1;
   for ODCTRL_OC0_Field use
     (Disable => 0,
      Enable => 1);

   --  ODCTRL_OC array
   type ODCTRL_OC_Field_Array is array (0 .. 15) of ODCTRL_OC0_Field
     with Component_Size => 1, Size => 16;

   --  Type definition for ODCTRL_OC
   type ODCTRL_OC_Field
     (As_Array : Boolean := False)
   is record
      case As_Array is
         when False =>
         ...
         when True =>
            --  OC as an array
            Arr : ODCTRL_OC_Field_Array;
      end case;
   end record
     with Unchecked_Union, Size => 16;

   for ODCTRL_OC_Field use record
      Val at 0 range 0 .. 15;
      Arr at 0 range 0 .. 15;
   end record;
   ...
   type ODCTRL_Register is record
      --  Pn.0 open-drain control
      OC             : ODCTRL_OC_Field := (As_Array => False, Val => 16#0#);
      --  unspecified
      Reserved_16_31 : HAL.UInt16 := 16#0#;
   end record
     with Volatile_Full_Access, Size => 32,
