...
package NRF_SVD.WDT is
   ...
   type CFG_Register is record
      ...
      --  Write-only. Watchdog register key
   ...
   --  WDT clock source
   type CLKSOURCE_CLKSOURCE_Field is
     (
      Ihrc,
      Hclk,
      Ilrc,
      Els Xtal)
     with Size => 2;
   for CLKSOURCE_CLKSOURCE_Field use
     (Ihrc => 0,
      Hclk => 1,
      Ilrc => 2,
      Els Xtal => 3);
   ...
   type CLKSOURCE_Register is record
      --  WDT clock source
      CLKSOURCE     : CLKSOURCE_CLKSOURCE_Field := NRF_SVD.WDT.Ihrc;
      --  unspecified
