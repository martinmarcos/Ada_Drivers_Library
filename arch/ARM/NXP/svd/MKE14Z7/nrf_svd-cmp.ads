...
package NRF_SVD.CMP is
   ...
   --  DAC output Enable
   type C1_DACOE_Field is
     (
      --  DAC output is disabled to go outside.
      Dacoe_0,
      --  DAC output is enabled to go outside.
      Dacoe_1)
     with Size => 1;
   for C1_DACOE_Field use
     (Dacoe_0 => 0,
      Dacoe_1 => 1);
   ...
   type CMP0_C1_Register is record
      ...
      --  DAC output Enable
      DACOE          : C1_DACOE_Field := NRF_SVD.CMP.Dacoe_0;
      --  unspecified
