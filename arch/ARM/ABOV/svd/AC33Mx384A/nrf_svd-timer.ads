...
package NRF_SVD.TIMER is
   ...
   for SR_Register use record
      ...
      QRF           at 0 range 4 .. 4;
      QDIRCH        at 0 range 5 .. 5;
      Reserved_6_31 at 0 range 6 .. 31;
   end record;

   --  Timer n Interrupt Enable Register
   type IER_Register is record
      --  Write-only. Counter overflow interrupt enable
      OVIE          : Boolean := False;
      --  Write-only. GRB Match interrupt enable
      MBIE          : Boolean := False;
      --  Write-only. GRA Match interrupt enablee
      MAIE          : Boolean := False;
      --  unspecified
      Reserved_3_3  : HAL.Bit := 16#0#;
      --  Write-only. Quadrature revolution interrupt enable
      QRIE          : Boolean := False;
      --  Write-only. Quadrature direction change interrupt enable
      QDIRCHIE      : Boolean := False;
      --  Write-only. Quadrature decoder error interrupt enable
      QERRIE        : Boolean := False;
      --  unspecified
   ...
   for IER_Register use record
      ...
      Reserved_3_3  at 0 range 3 .. 3;
      QRIE          at 0 range 4 .. 4;
      QDIRCHIE      at 0 range 5 .. 5;
      QERRIE        at 0 range 6 .. 6;
   ...
   T4_Periph : aliased TIMER_Peripheral
     with Import, Address => System'To_Address (16#40003080#);

   --  16-BIT TIMER
   T5_Periph : aliased TIMER_Peripheral
     with Import, Address => System'To_Address (16#400030A0#);

   --  16-BIT TIMER
   T6_Periph : aliased TIMER_Peripheral
     with Import, Address => System'To_Address (16#400030C0#);

   --  16-BIT TIMER
   T7_Periph : aliased TIMER_Peripheral
     with Import, Address => System'To_Address (16#400030E0#);

   --  16-BIT TIMER
   T8_Periph : aliased TIMER_Peripheral
     with Import, Address => System'To_Address (16#40003100#);

   --  16-BIT TIMER
   T9_Periph : aliased TIMER_Peripheral
     with Import, Address => System'To_Address (16#40003120#);
