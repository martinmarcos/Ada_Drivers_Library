...
package NRF_SVD.SCU_RESET is
   ...
   --  Enable Loss of DCO1 Clock Reset
   type RSTCON_LOCRSTEN_Field is
     (
      --  No reset when loss of DCO1 clock occur
      VALUE1,
      --  Reset when loss of DCO1 clock occur
      VALUE2)
     with Size => 1;
   ...
   --  Enable USIC01 SRAM Parity Error Reset
   type RSTCON_U1PERSTEN_Field is
     (
      --  No reset when USIC1 memory parity error occur
      VALUE1,
      --  Reset when USIC1 memory parity error occur
      VALUE2)
     with Size => 1;
   for RSTCON_U1PERSTEN_Field use
     (VALUE1 => 0,
      VALUE2 => 1);

   --  Enable MultiCAN+SRAM Parity Error Reset
   type RSTCON_MPERSTEN_Field is
     (
      --  No reset when MultiCAN+ memory parity error occur
      VALUE1,
      --  Reset when MultiCAN+ memory parity error occur
      VALUE2)
     with Size => 1;
   for RSTCON_MPERSTEN_Field use
     (VALUE1 => 0,
      VALUE2 => 1);

   --  Enable Loss of External Clock Reset
   type RSTCON_LOECRSTEN_Field is
     (
      --  No reset when loss of external clock occur
      VALUE1,
      --  Reset when loss of external clock occur
      VALUE2)
     with Size => 1;
   for RSTCON_LOECRSTEN_Field use
     (VALUE1 => 0,
      VALUE2 => 1);
   ...
   type RSTCON_Register is record
      ...
      --  Enable USIC01 SRAM Parity Error Reset
      U1PERSTEN      : RSTCON_U1PERSTEN_Field := NRF_SVD.SCU_RESET.VALUE1;
      --  Enable MultiCAN+SRAM Parity Error Reset
      MPERSTEN       : RSTCON_MPERSTEN_Field := NRF_SVD.SCU_RESET.VALUE1;
      --  Enable Loss of External Clock Reset
      LOECRSTEN      : RSTCON_LOECRSTEN_Field := NRF_SVD.SCU_RESET.VALUE1;
      --  unspecified
   ...
   for RSTCON_Register use record
      ...
      U1PERSTEN      at 0 range 4 .. 4;
      MPERSTEN       at 0 range 5 .. 5;
      LOECRSTEN      at 0 range 6 .. 6;
