...
package NRF_SVD.BCCU is
   ...
   type DEEN_EDE0_Field is
     (
      --  Dimming Engine is disabled;Channel brightness gets written with the
      --  value of channel intensity
      VALUE1,
   ...
   type DESTRCON_DE0S_Field is
     (
      ...
      --  target has been reached after the configured dimming period.
      VALUE2)
     with Size => 1;
   ...
   type CHCONFIG_Register is record
      --  Packer Threshold
      PKTH           : CHCONFIG_PKTH_Field := NRF_SVD.BCCU.VALUE3;
      --  Packer Enable
